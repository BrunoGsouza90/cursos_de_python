...
if resposta == 'S':
    ...
    try:
        ...
        print(f'Banco de Dados "{arquivo}" já existe!')
else:
    arquivo = str(input('Digite o nome do Banco de Dados: ')).strip().capitalize()
...
resposta = str(input('Quer ler o Banco de Dados? [S/N] ')).strip().upper()[0]
while resposta not in 'SN':
    print('Resposta Inválida!')
    resposta = str(input('Quer ler o Banco de Dados? [S/N] ')).strip().upper()[0]
if resposta == 'S':
...
resposta = str(input('Quer deletar o Banco de Dados? [S/N] ')).strip().upper()[0]
while resposta not in 'SN':
    print('Resposta Inválida!')
    resposta = str(input('Quer deletar o Banco de Dados? [S/N] ')).strip().upper()[0]
if resposta == 'S':
    os.remove("Curso de Python CFB Cursos/Banco de Dados/" + arquivo + '.txt')
